...
export const remove = async (tracksIds: string[]) => {
  ...
  const result = await dialog.showMessageBox(app.browserWindows.main, {
    ...
    title: 'Effacer morceau(x)?',
    message: `Êtes-vous sûr(e) de vouloir supprimer ${tracksIds.length} element(s) de votre librairie?`,
    type: 'warning'
  });
...
export const reset = async () => {
  try {
    const result = await dialog.showMessageBox(app.browserWindows.main, {
      buttons: ['Annuler', 'Effacer'],
      title: 'Effacer morceaux?',
      message: 'Êtes-vous sûr(e) de vouloir vider votre librairie ? Tous vos moceaux seront effacés.',
      type: 'warning'
    });
