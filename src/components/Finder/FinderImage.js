...
import FinderListImage from "./FinderListImage";
...
const FinderImage = () => {
  ...
  useEffect(() => {
    ...
    ipcRenderer.send(FETCH_DATA_FROM_STORAGE, "imagePaths");
    ...
    <>
      {!paths.length && !files.length && (
        <div className="centerContainer">
          <h1>Vous n'avez pas encore importer d'image</h1>
      ...
      {files.length > 0 && (
        <FinderListImage onClickBack={onClickBackHandler} files={files} folderName={folderName} fullPath={fullPath}></FinderListImage>
      )}
    </>
  )
}
